...
description: "Read Uniswap V3 and V4 liquidity data on Robinhood Chain through the LP Agent API"
---

## Overview

LP Agent indexes Uniswap V3 and Uniswap V4 liquidity on **Robinhood Chain**, an EVM
network. The public API serves that data through the same endpoints you already use for
Solana: you select the network with a `chain` query parameter.

| | |
| --- | --- |
| Network | Robinhood Chain |
| Chain ID | `4663` |
| Protocols | Uniswap V3 (`uniswap_v3`), Uniswap V4 (`uniswap_v4`) |
...
<AccordionGroup>
  <Accordion title="Available on both SOL and ROBINHOOD">
    ...
    `/pools/{poolId}/positions` needs `chain=ROBINHOOD` for a Robinhood pool. On Robinhood
    its `positionState` is always empty and `activeBin` is omitted, since both are Meteora
    DLMM bin data, and `platform` is ignored in favour of the pool's own protocol.

    `/pools/{poolId}/onchain-stats` and `/pools/{poolId}/top-lpers` accept `chain` and
    `platform` like the other endpoints. `chain` is optional there: a Uniswap V3 pool address
    or Uniswap V4 pool ID (the 32-byte `0x` hash) is treated as Robinhood when you leave it
    out. Either ID works in any letter case.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Quote tokens for pool discovery">
    ...
    Pass the **symbol** (`quote_token=USDG`), not the address.
  </Accordion>

  <Accordion title="Pool discovery filters on Robinhood">
    `GET /pools/discover?chain=ROBINHOOD` differs from Solana in a few ways:

    - `min_bin_step`, `max_bin_step`, `min_organic_score` and `max_organic_score` are
      Solana concepts and are ignored.
    - Browsing hides pools under $5,000 liquidity. Pass `min_liquidity`,
      `show_small_pools=true` or `search` to see them.
    - Browsing also hides pools that claim at least $50,000 TVL on under $500 of 24h
      volume, the signature of TVL painted by a single-sided deposit. `search` skips this.
    - `category=stock` narrows to pools holding a tokenized stock from Robinhood's
      on-chain asset registry.
    - `search` matches `0x` addresses and Uniswap V4 pool IDs in any letter case.
  </Accordion>
  ...
  <Accordion title="Protocol defaults per chain">
    Endpoints that take a `platform` parameter default to every protocol served on the
    requested chain: `meteora,meteora_damm_v2` on `SOL`, `uniswap_v3,uniswap_v4` on
    `ROBINHOOD`. Pass it to narrow the result. The older names `protocol` (overview,
    revenue) and `type` (discover) are still accepted.
  </Accordion>
</AccordionGroup>
