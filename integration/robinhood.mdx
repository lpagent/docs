...
<AccordionGroup>
  <Accordion title="Available on both SOL and ROBINHOOD">
    ...
    `/pools/{poolId}/top-lpers` takes no `chain` parameter. The pool ID picks the chain:
    a Uniswap V3 pool address or a Uniswap V4 pool ID (the 32-byte `0x` hash) returns
    Robinhood LPers. Send the ID in lowercase; a checksummed address matches nothing and
