...
<AccordionGroup>
  <Accordion title="Available on both SOL and ROBINHOOD">
    ...
    `/pools/{poolId}/top-lpers` accepts `chain` and `platform` like the other endpoints.
    `chain` is optional there: a Uniswap V3 pool address or Uniswap V4 pool ID (the 32-byte
    `0x` hash) is treated as Robinhood when you leave it out. Either ID works in any letter
    case.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Protocol defaults per chain">
    Endpoints that take a `platform` or `protocol` parameter default to every protocol
    served on the requested chain: `meteora,meteora_damm_v2` on `SOL`,
    `uniswap_v3,uniswap_v4` on `ROBINHOOD`. Pass the parameter to narrow it to one.
  </Accordion>
</AccordionGroup>
